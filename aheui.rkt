...
;       (read-line (current-input-port))
	...
	(cond ((equal? cmd0 "ㅇ") (void))
		 ...
		 (cond ((= storage-no 21) (vector-set! storage storage-no (drop-right cur-storage 1)))
	      ...
	      ((equal? cmd0 "ㅂ") (cond ((equal? cmd2 "ㅇ") (vector-set! storage storage-no (cons (string->number (read-line (current-input-port))) cur-storage)))
					((equal? cmd2 "ㅎ") (vector-set! storage storage-no (cons (get-char-value (read-char (current-input-port))) cur-storage)))
	      ...
	      ((equal? cmd0 "ㅈ") (unless (= (length cur-storage) 0)
				   (cond ((= storage-no 21) (cond ((<= (length cur-storage) 1) (set! temp-value 1))
                                                             ((>= (car (cdr (reverse cur-storage))) (last cur-storage)) (set! temp-value 1))
                                                             (else (set! temp-value 0))))
					 (else (cond ((<= (length cur-storage) 1) (set! temp-value 1))
                                                ((>= (car (cdr cur-storage)) (car cur-storage)) (set! temp-value 1))
                                                (else (set! temp-value 0)))))
                              (if (<= (length cur-storage) 1) (vector-set! storage storage-no '())
