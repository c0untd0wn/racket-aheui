...
(define (run-aheui cmds pos-x pos-y prev-x prev-y direction storage-no storage)
  ...
  (if (char-not-in-boundary current-char)
      ;handle characters out of boundary (not a hangul character)
       (if (= (remainder direction 2) 1) (run-aheui cmds (+ pos-x direction) pos-y pos-x pos-y direction storage-no storage)
             (run-aheui cmds prev-x prev-y pos-x (+ pos-y (/ direction 2)) direction storage-no storage))
       (begin 
